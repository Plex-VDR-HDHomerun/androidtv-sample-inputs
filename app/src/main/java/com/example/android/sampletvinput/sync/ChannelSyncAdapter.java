...
import com.example.android.sampletvinput.player.utils.IptvUtil;
import com.example.android.sampletvinput.utils.TvContractUtils;
import com.example.android.sampletvinput.xmltv.XmlTvParser;
import com.example.android.sampletvinput.service.ITvListing;
...
public class ChannelSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority,
        ...
        LongSparseArray<XmlTvParser.Channel> channelMap = TvContractUtils.buildChannelMap(
     ...
     * @param channel The {@link com.example.android.sampletvinput.xmltv.XmlTvParser.Channel} for the programs to return.
     ...
     */
    private List<Program> getPrograms(Uri channelUri, XmlTvParser.Channel channel,
                                      List<XmlTvParser.Program> programs, long startTimeMs, long endTimeMs) {
        if (startTimeMs > endTimeMs) {
            throw new IllegalArgumentException();
        }
        List<XmlTvParser.Program> channelPrograms = new ArrayList<>();
        for (XmlTvParser.Program program : programs) {
        ...
        if (!channel.repeatPrograms) {
            for (XmlTvParser.Program program : channelPrograms) {
        ...
        while (programStartTimeMs < endTimeMs) {
            XmlTvParser.Program programInfo = channelPrograms.get(i++ % programCount);
    ...
    private void updatePrograms(Uri channelUri, List<Program> newPrograms) {
        ...
        List<Program> oldPrograms = TvContractUtils.getPrograms(context.getContentResolver(),
                channelUri);
