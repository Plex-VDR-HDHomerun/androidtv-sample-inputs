...
import com.example.android.sampletvinput.player.video.MediaSourceFactory;
import at.pansy.iptv.player.HlsRendererBuilder;
import com.example.android.sampletvinput.player.LeanbackPlayer;
import com.example.android.sampletvinput.player.utils.RendererUtil;
import com.example.android.sampletvinput.utils.SyncUtil;
...
public class TvInputService extends android.media.tv.TvInputService {
    ...
    class TvInputSession extends Session implements Handler.Callback {
        ...
        protected LeanbackPlayer player;
        ...
        private final LeanbackPlayer.Listener playerListener = new LeanbackPlayer.Listener() {
            ...
            public void onPrepared() {
                ...
                for (int i = 0; i < player.getTrackCount(LeanbackPlayer.TYPE_AUDIO); i++) {
                    MediaFormat format = player.getTrackFormat(LeanbackPlayer.TYPE_AUDIO, i);
                    ...
                    tracks.add(new TvTrackInfo.Builder(TvTrackInfo.TYPE_AUDIO,
                            ...
                            .build());
                    if (player.getSelectedTrack(LeanbackPlayer.TYPE_AUDIO) == i) {
                        audioSelected = Integer.toString(i);
                    }
                }

                for (int i = 0; i < player.getTrackCount(LeanbackPlayer.TYPE_VIDEO); i++) {
                    ...
                    tracks.add(new TvTrackInfo.Builder(TvTrackInfo.TYPE_VIDEO,
                            ...
                            .build());
                    if (player.getSelectedTrack(LeanbackPlayer.TYPE_VIDEO) == i) {
                        videoSelected = Integer.toString(i);
                    }
                }

                for (int i = 0; i < player.getTrackCount(LeanbackPlayer.TYPE_TEXT); i++) {
                    tracks.add(new TvTrackInfo.Builder(TvTrackInfo.TYPE_SUBTITLE,
                            Integer.toString(i))
                            .build());
                    if (player.getSelectedTrack(LeanbackPlayer.TYPE_TEXT) == i) {
        ...
        LeanbackPlayer.CaptionListener captionListener = new LeanbackPlayer.CaptionListener() {
            @Override
        ...
        private boolean playProgram(PlaybackInfo info) {
            ...
            LeanbackPlayer.RendererBuilder rendererBuilder;
            ...
            player = new LeanbackPlayer(rendererBuilder);
        ...
        private class PlayCurrentProgramRunnable implements Runnable {
            ...
            public void run() {
                ...
                if (programs.isEmpty()) {
                    ...
                    Channel channel = TvContractUtils.getChannel(context.getContentResolver(), mChannelUri);
                    if (channel != null) {
